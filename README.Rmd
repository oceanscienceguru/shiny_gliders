...
The goal of this tool is to provide glider pilots with additional information about vehicle health and science data. As well, it is a shareable platform built on R Shiny that can be used to keep non-pilot collaborators informed of mission progress without having to grant access to the piloting server.
...
Next, the R conversion script requires a similar, separate folder structure (see below). Within each glider's folder, two .csv files are maintained and a .RData containing all of the objects used for live data display.
...
Additionally, one may store example goto_l\*.ma files on the server in a folder named "routes under the top level directory. These can be used for quick checks our route planning using the routing module.
